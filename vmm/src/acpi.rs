...
fn create_acpi_tables_internal(
    ...
    numa_nodes: &NumaNodes,
    tpm_enabled: bool,
    #[cfg(feature = "fw_cfg")] add_to_fw_cfg: bool,
) -> (Rsdp, Vec<u8>) {
    ...
    // table is not offset for fw_cfg since fw will place acpi tables in memory
    ...
    if add_to_fw_cfg {
        ...
        let _ = device_manager
            ...
            .lock()
            .unwrap()
            .add_acpi(rsdp, table_bytes.clone(), checksums, pointers);
    }
    (rsdp, table_bytes)
}
...
pub fn create_acpi_tables_for_fw_cfg(
    ...
    let _ = create_acpi_tables_internal(
        dsdt_offset,
        device_manager,
        cpu_manager,
        memory_manager,
...
pub fn create_acpi_tables(
    ...
    let (rsdp, tables_bytes) = create_acpi_tables_internal(
        dsdt_offset,
        device_manager,
        cpu_manager,
        memory_manager,
    ...
    info!(
        ...
        Rsdp::len() as usize + tables_bytes.len(),
    );
