...
pub enum Error {
    ...
    FwCfgKernelFile(#[source] io::Error),

    #[error("Fw Cfg missing initramfs")]
    FwCfgInitramfs(#[source] io::Error),
...
impl Vm {
    ...
    pub fn new_from_memory_manager(
        ...
        #[cfg(all(feature = "fw_cfg", not(target_arch = "riscv64")))]
        if fw_cfg_config.is_some() {
            let fw_cfg_config = fw_cfg_config.unwrap();
