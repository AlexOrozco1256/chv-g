...
mod tests {
    ...
    fn test_seccomp_filter(
        ...
        unsafe {
            ...
            libc::ioctl(fd, VHOST_GET_FEATURES.try_into().unwrap(), 0);
            libc::ioctl(fd, VHOST_SET_FEATURES.try_into().unwrap(), 0);
            libc::ioctl(fd, VHOST_SET_OWNER.try_into().unwrap(), 0);
            libc::ioctl(fd, VHOST_SET_VRING_NUM.try_into().unwrap(), 0);
            libc::ioctl(fd, VHOST_SET_VRING_ADDR.try_into().unwrap(), 0);
            libc::ioctl(fd, VHOST_SET_VRING_BASE.try_into().unwrap(), 0);
            libc::ioctl(fd, VHOST_SET_VRING_KICK.try_into().unwrap(), 0);
            libc::ioctl(fd, VHOST_SET_VRING_CALL.try_into().unwrap(), 0);
            libc::ioctl(fd, VHOST_SET_BACKEND_FEATURES.try_into().unwrap(), 0);
            libc::ioctl(fd, VHOST_GET_BACKEND_FEATURES.try_into().unwrap(), 0);
            libc::ioctl(fd, VHOST_VDPA_GET_DEVICE_ID.try_into().unwrap(), 0);
            libc::ioctl(fd, VHOST_VDPA_GET_STATUS.try_into().unwrap(), 0);
            libc::ioctl(fd, VHOST_VDPA_SET_STATUS.try_into().unwrap(), 0);
            libc::ioctl(fd, VHOST_VDPA_GET_CONFIG.try_into().unwrap(), 0);
            libc::ioctl(fd, VHOST_VDPA_SET_CONFIG.try_into().unwrap(), 0);
            libc::ioctl(fd, VHOST_VDPA_SET_VRING_ENABLE.try_into().unwrap(), 0);
            libc::ioctl(fd, VHOST_VDPA_GET_VRING_NUM.try_into().unwrap(), 0);
            libc::ioctl(fd, VHOST_VDPA_SET_CONFIG_CALL.try_into().unwrap(), 0);
            libc::ioctl(fd, VHOST_VDPA_GET_IOVA_RANGE.try_into().unwrap(), 0);
            libc::ioctl(fd, VHOST_VDPA_GET_CONFIG_SIZE.try_into().unwrap(), 0);
            libc::ioctl(fd, VHOST_VDPA_SUSPEND.try_into().unwrap(), 0);
