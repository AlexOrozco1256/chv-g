...
pub enum Error {
    ...
    #[cfg(feature = "fw_cfg")]
    /// Failed Parsing FwCfgConfig
    #[error("Error parsing --fw-cfg-config: missing --kernel")]
    ParseFwCfgConfigKernel,
    #[cfg(feature = "fw_cfg")]
    /// Failed Parsing FwCfgConfig
    #[error("Error parsing --fw-cfg-config: missing --cmdline")]
    ParseFwCfgConfigCmdline,
    #[cfg(feature = "fw_cfg")]
    /// Failed Parsing FwCfgConfig
    #[error("Error parsing --fw-cfg-config: missing --initramfs")]
    ParseFwCfgConfigInitramfs,
}
...
impl VmConfig {
    ...
    pub fn parse(vm_params: VmParams) -> Result<Self> {
        ...
        let fw_cfg_config = if vm_params.fw_cfg_config.is_some() {
            let kernel = vm_params.kernel.is_some();
            let cmdline = vm_params.cmdline.is_some();
            let initramfs = vm_params.initramfs.is_some();
            let fw_cfg_config = FwCfgConfig::parse(vm_params.fw_cfg_config.unwrap())?;
            if fw_cfg_config.kernel && !kernel {
                return Err(Error::ParseFwCfgConfigKernel);
            } else if fw_cfg_config.cmdline && !cmdline {
                return Err(Error::ParseFwCfgConfigCmdline);
            } else if fw_cfg_config.initramfs && !initramfs {
                return Err(Error::ParseFwCfgConfigInitramfs);
            }
            Some(fw_cfg_config)
        } else {
            None
            ...
            FwCfgConfig {
                items: Some(FwCfgItemList {
                    item_list: vec![FwCfgItem {
                        name: Some("opt/org.test/fw_cfg_test_item".to_string()),
        ...
        assert_eq!(
            ...
            FwCfgConfig {
                items: Some(FwCfgItemList {
                    ...
                    FwCfgItem {
                        name: Some("opt/org.test/fw_cfg_test_item2".to_string()),
